...
class Admin_controller extends Controller {
    ...
    public function add_requirement_prev() {
 ...
 public function update_new_requirement_info(){
      $model = $this->loadModel('admin_model'); 
      $this->tenant_name = $_POST['tenant_name'];
     //$this->town = explode(",", $_POST['townvalue']);
...
public function history_info_for_property($property_id){
         ...
         if(empty(($result) && ($result2) && ($rsresult)))
               {
    ...
    <body class="main">
        <div class="book">  
            ...
            try {
               ...
               // $path=str_replace('application/controllers/admin_controller.php','',__FILE__).'pdf/';
                $outputfilemain = 'E:/xampp/htdocs/ideascompany/pdf/' . $outputfile . '.pdf';
...
$mail->setFrom('admin@harkalm.ideascompany.co.uk', 'Harkalm Investments');
$mail->addReplyTo('admin@harkalm.ideascompany.co.uk', 'Harkalm Investments');
$mail->addAttachment($outputfilemain);
$mail->isHTML(true); 
$mail->Subject = "Harkalm Property Alert";
$mail->Body    = $_POST['data']['emailbody'];
...
//if(!$mail->send()) {
   ...
   // echo 'Mailer Error: ' . $mail->ErrorInfo;
//} else {
    $result = $model->save_requirement_send_date($reqid,$property_id);
...
//
   ...
   public function history_info_for_requirement($requirement_id){
        $model = $this->loadModel('admin_model');
         ...
         if(empty($result))
               {
              echo 'Sorry, there is no pdf history.';
               } 
               else{
