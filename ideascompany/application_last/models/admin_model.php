...
public function upload_update($image){
       ...
       $imagename = rand(5, 15) . basename($image["name"]);       
       $file_name=time() . rand(99, 9999) . $imagename;
...
//    }
    ...
    public function getrequirements($requirement_id) {
        $sql = "SELECT tbl_requirement.*,tbl_agent.agent_email, tbl_agent.is_primary  FROM  tbl_requirement LEFT JOIN requirement_agent ON requirement_agent.requirement_id=tbl_requirement.requirement_id LEFT JOIN tbl_agent ON requirement_agent.agent_id=tbl_agent.agent_id  WHERE tbl_requirement.requirement_id=" . $requirement_id . " AND tbl_agent.is_primary=1 GROUP BY tbl_requirement.requirement_id";
        $result = $this->querynr($sql);
...
public function update_agent_info_by_ajax($data,$agent_id){ 
  ...
  public function select_pdf_history_for_requirement($requirement_id){
     $result =$this->query("SELECT tbl_requirement.*,property.*,tbl_property_requirement.*,tbl_pdf_info.requirement_id,tbl_pdf_info.property_id FROM  tbl_requirement,property,tbl_pdf_info,tbl_property_requirement WHERE tbl_requirement.requirement_id=tbl_pdf_info.requirement_id AND tbl_pdf_info.property_id=property.property_id AND tbl_property_requirement.requirement_id=tbl_requirement.requirement_id AND tbl_property_requirement.property_id=property.property_id  AND  tbl_requirement.requirement_id=" . $requirement_id);
    return $result;  
  }
